...
class TwoStageAmp(gym.Env):
    ...
    def __init__(self, multi_goal=False):
        design_specs_fname = "bag_deep_ckt/template_specs/opamp_two_stage_sim.yaml"
        ...
        self.observation_space = spaces.Box(
            low=np.append(np.array([TwoStageAmp.PERF_LOW]*2*len(self.specs)),-5.0*np.ones(self.num_params)),
            high=np.append(np.array([TwoStageAmp.PERF_HIGH]*2*len(self.specs)),5.0*np.ones(self.num_params)))
